  ...
  --compressed`
    ...
    if [[ $list =~ "blocked" ]]
      then
        ...
        echo $list | jq '.data.shoppingListDetails.items.listItems[].product.usItemId' >> 1-usItemId.txt
        echo $list2 | jq '.data.shoppingListDetails.items.listItems[].product.usItemId' >> 1-usItemId.txt
        echo $list | jq '.data.shoppingListDetails.items.listItems[].product.priceInfo.currentPrice.price' >> 1-currentPrice.txt
        echo $list2 | jq '.data.shoppingListDetails.items.listItems[].product.priceInfo.currentPrice.price' >> 1-currentPrice.txt
        echo $list | jq '.data.shoppingListDetails.items.listItems[].product.priceInfo.wasPrice.price' | sed 's/null//g' >> 1-wasPrice.txt
        echo $list2 | jq '.data.shoppingListDetails.items.listItems[].product.priceInfo.wasPrice.price' | sed 's/null//g' >> 1-wasPrice.txt
        echo $list | jq '.data.shoppingListDetails.items.listItems[].product.availabilityStatus' >> 1-availabilityStatus.txt
        echo $list2 | jq '.data.shoppingListDetails.items.listItems[].product.availabilityStatus' >> 1-availabilityStatus.txt
        echo ${array[0]}
        break
    fi
  done
...
linkPost=$linkApi`echo LinkID==IMPORTDATA\(\"`$linkGit`echo 1-usItemId.txt\"\)\
\&GiaHienTai==IMPORTDATA\(\"`$linkGit`echo 1-currentPrice.txt\"\)\
\&TrangThaiHang==IMPORTDATA\(\"`$linkGit`echo 1-availabilityStatus.txt\"\)\
\&GiaGoc==IMPORTDATA\(\"`$linkGit`echo 1-wasPrice.txt\"\)
`
